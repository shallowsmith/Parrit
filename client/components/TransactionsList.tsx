      ...
      {dateKeys.length > 1 && (
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginBottom: 8 }}>
          <TouchableOpacity onPress={() => setShowPreviousDays((s) => !s)} style={{ paddingHorizontal: 8, paddingVertical: 6 }}>
            <Text style={{ color: '#9CA3AF', fontSize: 13 }}>{showPreviousDays ? 'Hide previous days' : `Show previous days (${otherDaysCount})`}</Text>
          </TouchableOpacity>
        </View>
      )}
      {visibleKeys.map((dateKey) => (
        <View key={dateKey} style={{ width: '100%', marginTop: 12 }}>
          ...
          {groups[dateKey].map((tx: any) => {
            ...
            return (
              <TouchableOpacity key={tx.id || tx._id || Math.random()} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#10130F', borderRadius: 12, padding: 12, marginBottom: 8 }} onPress={() => onSelectTransaction?.(tx)}>
