export function mapTextToBucketByKeywords(text: string): string | null {
  ...
  // Groceries: includes fruits, vegetables, produce, and other grocery items
  if (/\b(grocery|groceries|grocer|supermarket|fruits?|veggies|vegetables?|produce|meat|dairy|milk|eggs|bread|cheese|yogurt|chicken|beef|pork|fish|seafood|organic|fresh)\b/.test(t)) return 'groceries';
  if (/\b(food|restaurant|coffee|cafe|deli|meal|eat|lunch|dinner|breakfast|snack|latte|espresso|burger|pizza|sandwich|starbucks)\b/.test(t)) return 'food';
